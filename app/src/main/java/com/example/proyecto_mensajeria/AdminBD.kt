...
class AdminBD(context: Context) : SQLiteOpenHelper(context, DataBase, null, 1) {
    ...
    override fun onCreate(db: SQLiteDatabase?) {
        ...
        db?.execSQL("create table grupo(_id_grupo text primary key,grupo int,nocontrol text,materiaid text,docenteid text," +
        ...
        db?.execSQL("create table mensajes(_id_mensaje text primary key,Emisor text,Receptor text,Mensaje text," +
